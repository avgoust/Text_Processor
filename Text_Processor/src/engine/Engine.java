...
public class Engine implements IPlainTextDocumentEngine {
	 ...
	 * @param pInputType the String characterizing the document as raw (DocumentRawType.RAW)
	 *  unless "ANNOTATED" is given as input, in which case, the input file is characterized 
	 *  as annotated (DocumentRawType.ANNOTATED)
	...
	public Engine(String pFilePath, String pInputType, String pAlias) {
		...
		this.lineblocks = this.document.getLineBlock();
		this.alias = pAlias;
	 ...
	 * 
	 * Returns a RuleSet object as the result of the parsing and internal representation of the 
	 ...
	 * Every category absent is setup to undefined status and eventually mapped to the resp. 
	 * RuleUndefined object that always returns false for isValid()
	...
	public RuleSet registerInputRuleSetForPlainFiles(List<List<String>> inputSpec) {
		...
		RuleSetCreator ruleSetCreator = new RuleSetCreator(lineblocks, inputSpec, "inputRuleSetCurrentBlock");
		this.inputRuleSet =  ruleSetCreator.createRuleSet();


		for(LineBlock element : lineblocks)
		{
			element.setFormat(inputRuleSet.determineFormatStatus(element));
			element.setStyle(inputRuleSet.determineHeadingStatus(element));
		}
	 ...
	 * The 3rd element is a prefix string for STARTS_WITH or a comma-separated string 
	 * of positions (starting from 0) for POSITIONS
	 * 
	 ...
	 */
	@Override
	public RuleSet registerInputRuleSetForAnnotatedFiles(
			List<List<String>> inputSpec, List<String> prefixes) {
		this.inputSpec = inputSpec;
		...
		RuleSetCreator ruleSetCreator = new RuleSetCreator(lineblocks, inputSpec, 
				"inputRuleSet");
		this.inputRuleSet =  ruleSetCreator.createRuleSet();
	...
	public int exportMarkDown(String outputFileName) {
		...
		System.out.println("[Engine.loadProcessMarkup] [file: " + simpleInputFileName 
	...
	private void loadRawDocument(String fileName) {
		RawFileLineLoader loader = new RawFileLineLoader();
		loader.fileLoad(this.filePath,  this.lineblocks);
	...
	private void characterizeLineblocks(Document document, RuleSet ruleSet) {
		List<LineBlock> lineblocks = document.getLineBlock();
	...
	public void rulesImplementation()
	{
		...
		for(LineBlock element : lineblocks)
		{
			for(String element2 : element.getLines())
			{
