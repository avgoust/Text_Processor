...
public class RuleSetCreator {
	...
	public RuleSet createRuleSet() {
		...
		for(List<String> element: inputSpec) {
			...
			switch(element.get(1).strip().toUpperCase()) {
				case "STARTS_WITH":	lastParameter = element.get(2);
									currentRule = factory.createRuleStartWith(lastParameter);
									break;
				case "ALL_CAPS":	currentRule = factory.createRuleAllCaps();
									break;
				case "POSITIONS":	lastParameter = element.get(2);
			...
			switch (element.get(0).strip().toUpperCase()) {
				case "OMIT" -> {
